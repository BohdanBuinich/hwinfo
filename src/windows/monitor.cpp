...
// Correct Usage: Convert `CHAR[128]` to `std::string`
std::string ConvertCharArrayToString(const char* input) {
  std::wstring wideStr = ConvertToWideString(input);  // Convert to wide first
  return ConvertToNarrowString(wideStr.c_str());      // Convert to narrow
}
...
std::vector<Monitor> getAllMonitors() {
  ...
  while (EnumDisplayDevices(NULL, deviceIndex, &displayDevice, 0)) {
    if (displayDevice.StateFlags & DISPLAY_DEVICE_ACTIVE) {
      ...
      std::string resolution = "Unknown", refreshRateStr = "Unknown";
