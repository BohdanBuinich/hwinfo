...
std::string getVendorFromEDID(const std::vector<uint8_t>& edid) {
  if (edid.size() < 128) return "<unknown>";
...
std::pair<std::string, std::string> getModelAndSerialFromEDID(const std::vector<uint8_t>& edid) {
  if (edid.size() < 128) return {"<unknown>", "<unknown>"};
...
std::vector<Monitor> getAllMonitors() {
  ...
  for (const auto& dri_path : drm_util::getAvailableDRICards()) {
    ...
    drmModeRes* res = drmModeGetResources(drmDevice.getFD());
    if (!res) continue;

    for (int i = 0; i < res->count_connectors; ++i) {
      drmModeConnector* conn = drmModeGetConnector(drmDevice.getFD(), res->connectors[i]);
      if (!conn) continue;
      ...
      if (conn->connection == DRM_MODE_CONNECTED && conn->count_modes > 0) {
        ...
        std::string resolution =
            std::to_string(conn->modes[0].hdisplay) + "x" + std::to_string(conn->modes[0].vdisplay);
        std::string refreshRate = std::to_string(conn->modes[0].vrefresh);
